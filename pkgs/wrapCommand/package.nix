...
(symlinkJoin {
  ...
  postBuild = lib.concatMapStringsSep "\n" (
    ...
    let
      args =
        lib.optionals (commandName != "") [
          "--argv0"
          commandName
        ]
        ++ lib.optional inheritCommandName "--inherit-argv0"
        ++ lib.optionals (setEnvironment != { }) (
          lib.flatten (
        ...
        ++ lib.optionals (setEnvironmentDefault != { }) (
          lib.flatten (
            lib.mapAttrsToList (k: v: [
              "--set-default"
              k
        ...
        ++ lib.optionals (unsetEnvironment != [ ]) (
          lib.flatten (
            map (x: [
              "--unset"
              x
        ...
        ++ lib.optionals (workingDirectory != "") [
          "--chdir"
          workingDirectory
        ]
        ++ lib.optionals (beforeCommand != [ ]) (
          lib.flatten (
            map (x: [
              "--run"
              x
...
// {
  meta = package.meta // {
    description = "${package.meta.description} (wrapped)";
  };
}
