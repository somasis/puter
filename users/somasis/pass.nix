...
let
  qute-pass = pkgs.qute-pass.override {
    dmenu-pass = config.programs.dmenu-pass.package;
...
in
{
  ...
  cache.directories = lib.optional config.programs.dmenu-pass.enable {
    method = "symlink";
    directory = config.lib.somasis.xdgCacheDir "dmenu-pass";
  };
  ...
  programs.password-store = {
    ...
    package =
      ((if config.xsession.enable then pkgs.pass else pkgs.pass-wayland).override {
        ...
        (
          exts: with exts; [
            ...
            pkgs.passExtensions.pass-meta
            # pkgs.passExtensions.pass-link
          ]
        );
  };

  programs.dmenu-pass = {
    inherit (config.programs.dmenu) enable;
    settings.DMENU = ''dmenu -p pass -S'';
  };
  ...
  home.packages =
    ...
    ++ (lib.optional
      (
        config.programs.qutebrowser.enable
        && config.programs.password-store.enable
        && config.programs.dmenu-pass.enable
      )
      qute-pass);
