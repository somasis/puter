...
let
  retroarchUser = "tv";
  retroarchHome = config.users.users.${retroarchUser}.home;
  retroarchCache = "${retroarchHome}/.cache/retroarch";
  retroarchConfig = "${retroarchHome}/.config/retroarch";
  retroarchData = "${retroarchHome}/.local/share/retroarch";
  ...
  settings = rec {
    ...
    rgui_browser_directory = "${retroarchHome}/roms";
    ...
    system_directory = "${retroarchHome}/roms/system";
  ...
  retroarch =
    ...
    assert (config.services.udev.enable && config.hardware.uinput.enable);
    assert (builtins.elem "input" config.users.users.${retroarchUser}.extraGroups);
...
in
{
  ...
  # Allow tv to manage bluetooth things, and use controllers
  users.users.${retroarchUser}.extraGroups = [ "input" ];
