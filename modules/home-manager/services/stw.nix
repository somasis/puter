...
let
  ...
  intOrPercentType = types.oneOf [
    (types.strMatching "^-?(100|[0-9][0-9]?)%$|^-0$")
    types.int
  ];
  ...
  mkScript =
    ...
    let
      args =
        ...
        lib.cli.toGNUCommandLineShell { } {
          t = window.top;
          inherit (window.position) x;
          inherit (window.position) y;
    ...
    pkgs.writeShellScriptBin "stw-widget-${widget.name}" ''
      ${lib.optionalString (
        widget.update == 0
      ) "[[ -v NOTIFY_SOCKET ]] && ${pkgs.systemd}/bin/systemd-notify --ready"}
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    home.packages = [ pkg ] ++ (lib.mapAttrsToList (n: mkScript) cfg.widgets);
  };
}
